...
<head>
    <meta charset="UTF-8">
    <title>Bandas Piratas De One Piece</title>
...
$res->prepare("select count(*) as 'total' from bandaspiratas"); $res->execute(); $res->bind_result($total);
$fila = $res->fetch(); $numpag = ceil($total / $resultadopag); $pagactual = ($pag - 1) * $resultadopag;
$res->prepare("select * from gruposgobierno LIMIT " . $pagactual . ", " . $resultadopag); $res->execute();
...
if ($pag != 1) { echo " <a href='OP21.php?pag=" . ($pag - 1) . "'><</a> <a href='OP21.php?pag=1'>1</a> "; }
for ($i = $pag - 1; $i < $pag + 2; $i++) { if ($i > 1 && $i <= $numpag) { echo " <a href='OP21.php?pag=" . $i . "'>$i</a> "; } }
if ($pag != $numpag) { echo " ... <a href='OP21.php?pag=$numpag'>$numpag</a> <a href='OP21.php?pag=" . ($pag + 1) . "'>></a>"; }
