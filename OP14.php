...
<head>
    <meta charset="UTF-8">
    <title>Buscar Organizaciones De OP</title>
    <link rel="stylesheet" href="../ModelosEstilos/CSS2.css">
</head>

<body>
    <form action='OP14.php' method='post' style="text-align: center;">
        <input type="text" name="tiposdegrupos" placeholder="Tipo Organización">
        <input type="text" name="canon" placeholder="Subtipo (Canon/No Canon)">
        <input type="text" name="tipomiembro" placeholder="Tipo Miembro">
        <input type="text" name="titulogrupo" placeholder="Titulo Organización">
        <input type="text" name="grupo" placeholder="Organización">
        <input type="text" name="anteriorgrupo" placeholder="Anterior Organización">
        <input type="text" name="barco" placeholder="Barco"><br><br>
        <input type="submit" name="buscagrupo" value="Buscar Organización">
    </form> <?php
try { if (isset($_POST["buscagrupo"])) { $con = new PDO("mysql:host=localhost; dbname=onepiecedb", "root", "");
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); $con->exec("set character set utf8");
        $res = $con->prepare("SELECT * FROM organizaciones WHERE tiposdegrupos=? OR canon=? OR tipomiembro=? OR titulogrupo=?
            OR grupo=? OR anteriorgrupo=? OR barco=?"); $res->bindParam(1, $_POST["tiposdegrupos"]);
